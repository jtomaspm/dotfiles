...
FOLDER=$(ls -ld ~/.config/ ~/.config/*/ ~/Documents/code/ ~/Documents/code/*/ | awk '{print $9,$10}' | rofi -dmenu -p "Search > " )
...
if [[ ${#FOLDER} -gt 4 ]] ; then
    echo "Valid Folder"
    PROG=$(echo -e "open\ncode\ncreate folder" | rofi -dmenu -p "Search > ")
    echo "$PROG"
    if [ "$PROG" = "create folder" ] ; then
        FNAME=$(echo "" | rofi -dmenu -p "Folder Name > ")
        if [[ ${#FNAME} -gt 0 ]] ; then
